...
function getCompanyName(row: Record<string, unknown>): string {
...
export async function GET(request: Request) {
  ...
  if (error) return jsonError(error.message || 'Failed to load companies.', 500);
...
export async function POST(request: Request) {
  ...
  if (error && error.code === '42703') {
    const retry = await supabase.from('companies').insert({ name }).select('*').single();
    data = retry.data;
    error = retry.error;
  }

  if (error && error.code === '23505') {
    const existing = await supabase.from('companies').select('*').ilike('name', name).limit(1).maybeSingle();
    if (!existing.error && existing.data) {
      return NextResponse.json(existing.data, { status: 200 });
    }
  }

  if (error || !data) return jsonError(error?.message || 'Failed to create company.', 500);
